import { getOrCreateLocalInvoiceForJob, upsertRemoteInvoices } from '@/lib/offline/core-data';
...
export async function getOrCreateInvoiceForJob(userId: string, jobId: string): Promise<InvoiceRecord> {
  try {
    ...
    if (existing) {
      await upsertRemoteInvoices([existing]);
      return existing;
    }
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      if (!error) {
        await upsertRemoteInvoices([data]);
        return data;
      }
    ...
    return getOrCreateLocalInvoiceForJob(userId, jobId);
  }
}
