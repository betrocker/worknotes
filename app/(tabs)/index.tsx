...
export default function TabOneScreen() {
  ...
  const heroTitleTranslateY = useRef(new Animated.Value(14)).current;
  const heroTitleScale = useRef(new Animated.Value(0.97)).current;
  const heroGreetingOpacity = useRef(new Animated.Value(0)).current;
  const heroGreetingTranslateY = useRef(new Animated.Value(16)).current;
  const heroSublineOpacity = useRef(new Animated.Value(0)).current;
  const heroSublineTranslateY = useRef(new Animated.Value(18)).current;
  ...
  const clientsCardEnter = useRef(new Animated.Value(18)).current;
  const clientsCardOpacity = useRef(new Animated.Value(0)).current;
  const clientsCardScale = useRef(new Animated.Value(0.94)).current;
  const jobsCardEnter = useRef(new Animated.Value(18)).current;
  const jobsCardOpacity = useRef(new Animated.Value(0)).current;
  const jobsCardScale = useRef(new Animated.Value(0.94)).current;
  const debtCardEnter = useRef(new Animated.Value(22)).current;
  const debtCardOpacity = useRef(new Animated.Value(0)).current;
  const debtCardScale = useRef(new Animated.Value(0.94)).current;
  const servicesCardEnter = useRef(new Animated.Value(22)).current;
  const servicesCardOpacity = useRef(new Animated.Value(0)).current;
  const servicesCardScale = useRef(new Animated.Value(0.94)).current;
  ...
  const theme = useMemo(
    () => ({
      rootGradient: isDark ? ['#06111F', '#0D1F3B', '#132A4F', '#0B1220'] : ['#1A4FE0', '#3A70EE', '#7EA6FF', '#E9EEFF'],
      rootLocations: isDark ? [0, 0.28, 0.62, 1] : [0, 0.22, 0.55, 1],
      heroOverlay: isDark ? ['rgba(56,106,255,0.28)', 'rgba(12,58,180,0)'] : ['rgba(12,58,180,0.52)', 'rgba(12,58,180,0)'],
      ...
      sheetGradient: isDark
        ? ['rgba(24,29,42,0.98)', 'rgba(19,24,36,0.97)', 'rgba(15,19,30,0.98)']
        : ['rgba(250,252,255,0.96)', 'rgba(241,246,255,0.94)', 'rgba(233,239,255,0.92)'],
      sheetHighlight: isDark
        ? ['rgba(255,255,255,0.08)', 'rgba(255,255,255,0.03)', 'rgba(255,255,255,0)']
        : ['rgba(255,255,255,0.98)', 'rgba(255,255,255,0.7)', 'rgba(255,255,255,0.1)', 'rgba(255,255,255,0)'],
      sheetHighlightLocations: isDark ? [0, 0.28, 1] : [0, 0.12, 0.4, 1],
      ...
      sectionBg: isDark ? 'rgba(28,32,44,0.92)' : 'rgba(255,255,255,0.70)',
      sectionBorder: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.70)',
      sectionShadowOpacity: isDark ? 0.24 : 0.14,
      ...
      metricCards: {
        clients: isDark
          ? { colors: ['#1D2A44', '#162236'], iconBg: '#243A66', icon: '#8FB2FF', valueColor: '#9CB9FF' }
          : { colors: ['#D9EEFF', '#CCE4FF'], iconBg: '', icon: '#4A90E2', valueColor: '#4A70AA' },
        jobs: isDark
          ? { colors: ['#3A261E', '#2A1C17'], iconBg: '#4A2F23', icon: '#FFB067', valueColor: '#FFAF84' }
          : { colors: ['#FFE8D6', '#FBE2CF'], iconBg: '', icon: '#F08C45', valueColor: '#E26D45' },
        debts: isDark
          ? { colors: ['#3A3020', '#2B2418'], iconBg: '#4D3B1B', icon: '#FFD27A', valueColor: '#FFD27A' }
          : { colors: ['#FFF2D9', '#FCEBCB'], iconBg: '#FFBE55', icon: '#8A5100', valueColor: '#A36B11' },
        upcoming: isDark
          ? { colors: ['#1E322E', '#172723'], iconBg: '#21443C', icon: '#7FD4B8', valueColor: '#7FD4B8' }
          : { colors: ['#E6F6F3', '#DFF0EA'], iconBg: '#9CD8C8', icon: '#2B6A5A', valueColor: '#2B6A5A' },
      },
      actionCards: isDark
        ? {
            job: ['#1B2942', '#162033'],
            client: ['#1C3027', '#16251E'],
            debts: ['#392621', '#2C1D19'],
            jobs: ['#2B2440', '#211B31'],
  ...
  const runHeroTextIn = useCallback(() => {
    ...
    heroTitleTranslateY.setValue(14);
    heroTitleScale.setValue(0.97);
    heroGreetingOpacity.setValue(0);
    heroGreetingTranslateY.setValue(16);
    heroSublineOpacity.setValue(0);
    heroSublineTranslateY.setValue(18);
    mascotOpacity.setValue(0);
    mascotTranslateY.setValue(16);
    mascotScale.setValue(0.94);
    ...
    return Animated.sequence([
      ...
      Animated.parallel([
        Animated.timing(heroGreetingOpacity, {
          toValue: 1,
          duration: 220,
          easing: Easing.out(Easing.cubic),
          useNativeDriver: true,
        }),
        Animated.timing(heroGreetingTranslateY, {
          toValue: 0,
          duration: 280,
          easing: Easing.out(Easing.cubic),
          useNativeDriver: true,
      ...
      Animated.parallel([
        Animated.timing(heroSublineOpacity, {
          toValue: 1,
          duration: 200,
          easing: Easing.out(Easing.cubic),
          useNativeDriver: true,
        }),
        Animated.timing(heroSublineTranslateY, {
          toValue: 0,
          duration: 260,
          easing: Easing.out(Easing.cubic),
          useNativeDriver: true,
  ...
  const runCardsFloatIn = useCallback(() => {
    ...
    resetCard(clientsCardEnter, clientsCardOpacity, clientsCardScale, 24);
    resetCard(jobsCardEnter, jobsCardOpacity, jobsCardScale, 24);
    resetCard(debtCardEnter, debtCardOpacity, debtCardScale, 28);
    resetCard(servicesCardEnter, servicesCardOpacity, servicesCardScale, 28);
    ...
    return Animated.sequence([
      ...
      Animated.stagger(55, [
        ...
        animateCard(jobsCardEnter, jobsCardOpacity, jobsCardScale, 220, 180, 220),
        animateCard(debtCardEnter, debtCardOpacity, debtCardScale, 240, 190, 240),
        animateCard(servicesCardEnter, servicesCardOpacity, servicesCardScale, 240, 190, 240),
      ]),
    ]);
  }, [
    clientsCardEnter,
      ...
      <Animated.View
        ...
        <View
          ...
          <LinearGradient
            ...
            style={{ flex: 1 }}>
        <LinearGradient
          colors={theme.sheetHighlight}
          locations={theme.sheetHighlightLocations}
          start={{ x: 0.5, y: 0 }}
          end={{ x: 0.5, y: 1 }}
          pointerEvents="none"
          style={{
        ...
        <View
          ...
          style={{
            ...
            right: 16,
            height: 2,
            backgroundColor: theme.sheetTopLine,
            borderRadius: 999,
            opacity: isDark ? 0 : 1,
          }}
        />
        <ScrollView
          contentContainerStyle={{ paddingTop: 18, paddingHorizontal: 16, paddingBottom: 140 }}
        ...
        <Animated.View
          ...
          <Animated.View
            ...
            <LinearGradient
              ...
              <Text style={{ color: theme.metricCards.debts.valueColor, marginTop: 4, fontSize: 22, fontWeight: '800', lineHeight: 24 }}>
                {totalDebtLabel}
              </Text>
            </LinearGradient>
          </Animated.View>
          ...
          <Animated.View
            ...
            <LinearGradient
              ...
              <Text style={{ color: theme.metricCards.upcoming.valueColor, marginTop: 4, fontSize: 22, fontWeight: '800', lineHeight: 24 }}>
                {upcomingJobsCount ?? '—'}
              </Text>
            </LinearGradient>
          </Animated.View>
        ...
        <View
          className="mt-6 rounded-[24px] px-4 py-4"
          style={{
            ...
            todayJobs.slice(0, 3).map((job, index) => {
              ...
              return (
                <Pressable
                  ...
                  <LinearGradient
                    colors={isDark ? ['#202637', '#191F2D'] : ['#FFFFFF', '#F7F9FF']}
                    start={{ x: 0, y: 0 }}
                    end={{ x: 1, y: 1 }}
                    style={{
                      borderRadius: 20,
          ...
          {urgentDebtClients.length ? (
            urgentDebtClients.map((client, index) => (
              <Pressable
                ...
                <LinearGradient
                  colors={isDark ? ['#202637', '#191F2D'] : ['#FFFFFF', '#FBFCFF']}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 1 }}
                  style={{
                    borderRadius: 20,
        ...
        <View className="mt-4">
          <Text style={{ color: theme.title, marginBottom: 12, fontSize: 18, fontWeight: '800' }}>{t('home.quickActions')}</Text>
